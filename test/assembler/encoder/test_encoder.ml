...
let test_encode_instruction instruction (expected_upper, expected_lower) =
  let test_bytes = Bytes.create 2 in
  let test_bytes, _ = Encoder.encode_instruction (test_bytes, 0) instruction in
  Bytes.get test_bytes 0 = char_of_int expected_upper &&
  Bytes.get test_bytes 1 = char_of_int expected_lower
...
let () =
  if List.for_all test_instruction test_instruction_list then exit 0
  else exit 1
